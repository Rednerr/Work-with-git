...
void insert(tree*& tr, int x) {  // переименовал root -> tr
    if (tr == nullptr) {
        tr = ::root(x);  // теперь ::root(x) вызывает глобальную функцию
        return;
    }
...
int main(){
    ...
    std::vector<int> v = {10, 5, 15, 3, 7, 13, 18, 20, 1};
    tree* root = nullptr;
    for (int val : v) {
        insert(root, val);
    }
