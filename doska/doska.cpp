...
void Graph() {
    ...
    for (int y = 0; y < 8; y++) { // перебор строк
        for (int x = 0; x < 8; x++) {// столбцов
            int v = y * 8 + x; // номер клетки
            for (int i = 0; i < 8; i++) { // 8 ходов 
                int Ox = x + hod_x[i];  //нова€ строка 
                int Oy = y + hod_y[i];  //новый столбец 
                if (Ox >= 0 && Ox < 8 && Oy >= 0 && Oy < 8) { //не выходит за границы
...
int in_numbers(const string &kletka) {
    return (kletka[1] - '1') * 8 + (toupper(kletka[0]) - 'A');
}

string in_letters(int kletka) {
    string s;
    s += char('A' + (kletka % 8)); 
    s += char('1' + (kletka / 8));
    return s;
